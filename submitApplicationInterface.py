...
rowCount = 0
for row in open("resumeData.csv"):
    rowCount += 1

print("the number of rows in the file is", rowCount)

header = ['id','firstName', 'lastName','phone','email','education','skills']
data = [rowCount, '0', '0', '0', '0', '0', '0']
...
layout = [  
            ...
            sg.FileBrowse(initial_folder=working_directory, file_types=[("DOCX Files", "*.docx")])],
            [sg.Button('Submit'), sg.Exit()]
        ]

window = sg.Window("Application Submitter", layout)
...
def printDetails(docx_address):
    text = extractTextDocx(docx_address)
    ...
    data1 = [rowCount, '0', '0', '0', '0', '0', '0'] # A list that stores all of the data that is extraccted from a resume

    # Prints out the name found in the raw text if one is found
    if name:
        name = name.split(' ')
        if(len(name) <= 2):
            data1[1] = name[0] # Stores the first name
            data1[2] = name[1] # Stores the last name
            print('Name: ', name)
         
    # Prints out the email found in the raw text if one is found
    if(email):
        data1[3] = email # Stores the first email
        print('\n' + 'Email: ' + email + '\n')

    # Prints out the phone number found in the raw text if one is found
    if(phoneNumber):
        phoneNumber = re.sub(r'[^0-9]','',phoneNumber)
        data1[4] = phoneNumber # Stores the phone number
        print('Phone Number: ' + phoneNumber + '\n')
    
    # Prints out all of the educations found in the raw text if atleast one is found
    if (educations):
        print('Educations:')
        tempList = list(educations)
        if(len(tempList) <= 1):
                data1[5] = tempList[0] # Stores the education
                print(tempList)
        else:
            tempString = ""
            for index in tempList:
                ...
                print(index)
            data1[5] = tempString # Stores the education(s)
    ...
    if(skills):
        ...
        tempString2 = ""
        for skill in skills:
            tempString2 = tempString2 + skill + "."
            print('- ' + skill)
        data1[6] = tempString2 # Stores the skill(s)

    # Saves the list data1 to the csv file resumeData.csv
    with open('resumeData.csv', 'a', encoding='UTF8', newline = '') as file:
...
while True:
    ...
    if event in (sg.WIN_CLOSED, 'Exit'):
        ...
        docx_address = values["-FILE_PATH-"]
        printDetails(docx_address)  
window.close()
